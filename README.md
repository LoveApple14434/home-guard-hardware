...
| SP3T 门锁 | GPIO38/39/40 + GPIO41/42/47 | 单刀三掷开关模拟门锁 + 三色 LED 指示（见 2.4 节） |
| 板载LED  | GPIO48| 状态指示灯（S3 DevKitC）    |

### 2.4 智能门锁模拟（SP3T 开关）

用**单刀三掷（SP3T）开关**模拟智能门锁的三种状态，无需真实锁体即可在 HA 中
测试门锁卡片与自动化：

| SP3T 开关位置 | GPIO | 门锁状态 | lock 实体状态 | text_sensor 显示 |
|---------------|------|----------|---------------|------------------|
| 位置 1（锁定）  | GPIO38 | 锁定     | `locked`      | 锁定 |
| 位置 2（解锁成功）| GPIO39 | 解锁成功 | `unlocked`    | 解锁成功 |
| 位置 3（解锁失败）| GPIO40 | 解锁失败 | `jammed`（卡住）| 解锁失败 |

**接线**：

1. SP3T 开关的公共端（COM）接 **3.3V**
2. 三个输出端分别接 **GPIO38 / GPIO39 / GPIO40**（已启用内部下拉，拨到哪一路哪一路读到高电平）
...
三个输出引脚各接一个 LED（**高电平点亮**），与开关位置同步显示当前状态：
...
同一时刻只有一个 LED 点亮（对应的那一盏），拨动开关即同步切换。
3. 无需外接电阻（内部下拉即可）

**说明**：HA 前端点击 lock/unlock 不会改变物理开关，状态始终反映开关位置；
