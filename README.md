...
| TOF050C  | GPIO8(SDA) + GPIO9(SCL)    | 激光测距模块（VL6180X, I2C, 见 2.7 节） |
...
**无自动锁定**：解锁成功 / 解锁失败状态会**一直保持**，直到手动按下对应按钮或通过 HA 前端操作切换。
...
### 2.7 激光测距（TOF050C，VL6180X）

用 **TOF050C 激光测距模块**（基于 ST VL6180X 直接 ToF 芯片）测量 **0~50cm** 的短距离，
以 `sensor` 实体注册到 HA，适合水位 / 门缝 / 限位 / 近距离避障检测。

**接线**（6 脚模块，I2C 地址 `0x29`，独立总线 `tof_i2c` 与 NFC 的 `nfc_i2c` 隔离）：

| 引脚 | 类型 | 接线 |
|------|------|------|
| VIN | 电源 | → 3.3V（模块支持 3.0~5.0V） |
| GND | 地 | → GND |
| SDA | I2C 数据 | → GPIO8（摄像头 D0 释放脚） |
| SCL | I2C 时钟 | → GPIO9（摄像头 D1 释放脚） |
| INT | 中断（可选） | 可不接 |
| SHUT | 关断（低电平有效） | ⚠️ **必须接 3.3V 拉高使能**；接低或悬空会“检测不到 / 一直读 0” |

> 也可在 YAML 里配 `enable_pin` 用 GPIO 控制 SHUT（如 GPIO13），此时无需硬件接 3.3V。

**使用说明**：

- 量程：默认 `range_scaler: 1x` 在 ~20cm 内最准（±5mm）；改 `2x` / `3x` 可扩到 ~40 / 60cm（精度下降）
